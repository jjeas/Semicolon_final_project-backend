...
public class KakaoAuthServiceImpl implements KakaoAuthService {
    ...
    public TokenResponseDTO login(String code) {
        ...
        // 1️⃣ code → 카카오 access token
        String kakaoAccessToken = getAccessToken(code);

        // 2️⃣ access token → 사용자 정보
        ...
        return new TokenResponseDTO(jwt, member.getMemberRole().name(),member.getMemberLoginId());
    }
    ...
    private KakaoUser getKakaoUser(String accessToken) {
        ...
        // 5️⃣ 생년월일 (birthyear + birthday → LocalDate)
